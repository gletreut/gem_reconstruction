...
#include "include/gem_contacts_kij.cpp"
...
//****************************************************************************
  int N,npos;
  double a,thres;
  string pathtocmat,pathtokmat,pathtokmat_in;
  ...
  gsl_matrix *kmat(0),*cmat(0), *kmatin(0);
  model *mod(0);
  workspace *wkp(0);
...
//****************************************************************************
  ...
  /* create matrices */
  kmatin = gsl_matrix_calloc(N+1,N+1);
  cmat = gsl_matrix_calloc(N+1,N+1);
  kmat = gsl_matrix_calloc(N+1,N+1);

  /* import contact map */
  fin.open(pathtokmat_in.c_str());
  load_matrix(fin, kmatin);
  fin.close();

  /* create workspace and model*/
  wkp = new workspace(N);
  mod = new model(kmatin, thres, contact_func_f, contact_func_df, contact_func_finv, wkp);
